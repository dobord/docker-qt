# Docker container to build Qt 6.3 for Linux 64-bit projects with latest cmake and linuxdeployqt
# Image: a12e/docker-qt:6.3-linux
...
&&  apt install -y --no-install-recommends \
    ...
    unixodbc-dev \
    zlib1g-dev \
&&  curl -Lo - https://download.qt.io/archive/qt/$(echo "${QT_VERSION}" | cut -d. -f 1-2)/${QT_VERSION}/single/qt-everywhere-src-${QT_VERSION}.tar.xz | tar xJ \
